...
export class NoProxyMatcher {
    ...
    bypass(hostname) {
        ...
        for (const rule of this.rules) {
            if (rule.kind === 'all')
                return true;
            if (rule.kind === 'hostname' && (host === rule.value || host.endsWith('.' + rule.value)))
                return true;
            if (rule.kind === 'ip' && family !== 0 && host === rule.value)
                return true;
            if (rule.kind === 'cidr4' && family === 4) {
                const mask = rule.bits === 0 ? 0 : (~0 << (32 - rule.bits)) >>> 0;
                if ((numeric & mask) === (rule.base & mask))
                    return true;
            }
        }
        return false;
